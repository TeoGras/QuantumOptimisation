...
def TSP(nbOfPoint, matrixOfCost, fileName):
    ...
    x = {(c, p): Binary("x{}_{}".format(c, p)) for c in range(nbOfPoint) for p in range(nbOfPoint + 1)}  # +1 cause depository take the first and last position

    # Objective function
    objective = quicksum(matrixOfCost[c1][c2] * x[(c1, p)] * x[(c2, p + 1)] for c1 in range(nbOfPoint) for c2 in range(nbOfPoint) for p in range(nbOfPoint))
    ...
    cqm.add_constraint(quicksum(x[(0, p)] for p in range(nbOfPoint + 1)) == 2)

    # Every position needs to get only 1 city
    for p in range(nbOfPoint):
        cqm.add_constraint(quicksum(x[(c, p)] for c in range(nbOfPoint)) == 1)

    # Every city needs to have only 1 position
    for c in range(1, nbOfPoint):
        cqm.add_constraint(quicksum(x[(c, p)] for p in range(nbOfPoint)) == 1)
    ...
    for i in range(0, len(listPositionsPerCluster)):
        ...
        for j in range(0, len(listPositionsPerCluster[i])):
            ...
            if j < len(listPositionsPerCluster[i]) - 1:
                ...
                plt.annotate(
                    "",
                    xy=(listCities[listPositionsPerCluster[i][j]]),
                    xytext=(listCities[listPositionsPerCluster[i][j + 1]]),
                    arrowprops=dict(arrowstyle="<-", color=colors[int(i * len(colors) / len(listPositionsPerCluster))], lw=2),
                )
